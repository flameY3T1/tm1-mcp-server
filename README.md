...
> **Production instances:** set `TM1_MODE=readonly` to hide all writing and
> destructive tools. The default (`readwrite`) exposes the full lifecycle,
> including cube/dimension deletion and cell writes.
...
- `tm1_audit_feeders` — Static heuristics (S1–S6) scan cube rules for overfeeding: wildcard brackets, feeders into consolidated 
...
- `tm1_get_server_info` — Return TM1 server identity + curated configuration (TI, Rules, MTQ, JobQueuing, Memory, Logging, HTTP, Security)
