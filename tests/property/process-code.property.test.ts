...
describe("Property 9: Prozesscode-Tab-Vollständigkeit", () => {
  ...
  it("getProcessCode returns exactly four fields, none undefined", async () => {
    const codeArb = fc.record({
      PrologProcedure: fc.string({ maxLength: 200 }),
      MetadataProcedure: fc.string({ maxLength: 200 }),
      DataProcedure: fc.string({ maxLength: 200 }),
      EpilogProcedure: fc.string({ maxLength: 200 }),
    });
    await fc.assert(
      fc.asyncProperty(codeArb, async (apiCode) => {
        const f = vi
          .fn()
          .mockResolvedValue(mockResp({ Name: "TestProcess", ...apiCode }));
...
describe("Property 10: Partielle Tab-Aktualisierung", () => {
  ...
  it("only specified tabs are sent in PATCH, unspecified tabs are omitted", async () => {
    ...
    const subsetArb = fc.subarray(
      ["prolog", "metadata", "data", "epilog"] as const,
      { minLength: 1 },
    );
    const codeValueArb = fc.string({ minLength: 1, maxLength: 100 });

    await fc.assert(
      fc.asyncProperty(subsetArb, codeValueArb, async (tabSubset, newCode) => {
        ...
        for (const tab of tabSubset)
          expect(body[apiFieldMap[tab]]).toBe(newCode);
        for (const tab of tabNames) {
          if (!tabSubset.includes(tab))
            expect(body[apiFieldMap[tab]]).toBeUndefined();
        }
      }),
      { numRuns: 100 },
    );
