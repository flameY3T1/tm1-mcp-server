...
describe("Property 8: Prozessparameter-Roundtrip", () => {
  ...
  it("written parameters are identical to read parameters", async () => {
    ...
    await fc.assert(
      fc.asyncProperty(
        fc.array(paramArb, { minLength: 0, maxLength: 5 }),
        async (params) => {
          const apiReadResponse = {
            value: params.map((p) => ({
              Name: p.name,
              Type: p.type,
          ...
          const f = vi
            ...
            .mockImplementation((_url: string, opts: { method?: string }) => {
              ...
              return Promise.resolve({
                ...
                json: vi.fn().mockResolvedValue(apiReadResponse),
              } as unknown as Response);
            });
          ...
          // Read
          const readParams =
            await client.processes.getParameters("TestProcess");
