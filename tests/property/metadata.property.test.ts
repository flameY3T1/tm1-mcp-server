...
describe("Property 4: Metadaten-Antwort-Vollständigkeit", () => {
  ...
  it("getCubes returns all cubes with name and dimensions", async () => {
    ...
    await fc.assert(
      fc.asyncProperty(
        fc.array(cubeArb, { minLength: 0, maxLength: 10 }),
        async (apiCubes) => {
          ...
          for (let i = 0; i < apiCubes.length; i++) {
            ...
            expect(cubes[i].dimensions).toEqual(
              apiCubes[i].Dimensions.map((d) => d.Name),
            );
          }
        },
      ),
  ...
  it("getDimensions returns all dimensions with name and hierarchies", async () => {
    ...
    await fc.assert(
      fc.asyncProperty(
        fc.array(dimArb, { minLength: 0, maxLength: 10 }),
        async (apiDims) => {
          ...
          for (let i = 0; i < apiDims.length; i++) {
            ...
            expect(dims[i].hierarchies).toEqual(
              apiDims[i].Hierarchies.map((h) => h.Name),
            );
          }
        },
      ),
  ...
  it("getProcesses returns all processes with name and parameters", async () => {
    const paramArb = fc.record({
      ...
      Type: fc.constantFrom("Numeric", "String"),
      Value: fc.oneof(fc.string({ maxLength: 30 }), fc.integer()),
    });
    const processArb = fc.record({
      Name: fc.string({ minLength: 1, maxLength: 30 }),
      Parameters: fc.array(paramArb, { minLength: 0, maxLength: 5 }),
    });
    await fc.assert(
      fc.asyncProperty(
        fc.array(processArb, { minLength: 0, maxLength: 10 }),
        async (apiProcs) => {
          ...
          for (let i = 0; i < apiProcs.length; i++) {
            ...
            expect(procs[i].parameters).toHaveLength(
              apiProcs[i].Parameters.length,
            );
            for (let j = 0; j < apiProcs[i].Parameters.length; j++) {
              expect(procs[i].parameters[j].name).toBe(
                apiProcs[i].Parameters[j].Name,
              );
              expect(procs[i].parameters[j].type).toBe(
                apiProcs[i].Parameters[j].Type,
              );
              expect(procs[i].parameters[j].defaultValue).toBe(
                apiProcs[i].Parameters[j].Value,
              );
            }
          }
        },
  ...
  it("getChores returns all chores with name, schedule, and processes", async () => {
    ...
    const choreArb = fc.record({
      Name: fc.string({ minLength: 1, maxLength: 30 }),
      Active: fc.boolean(),
      StartTime: fc.constant("2024-01-01T00:00:00"),
      DSTSensitive: fc.boolean(),
      Frequency: fc.constantFrom("P1D", "PT1H", "P7D"),
      Tasks: fc.array(taskArb, { minLength: 0, maxLength: 3 }),
    });
    await fc.assert(
      fc.asyncProperty(
        fc.array(choreArb, { minLength: 0, maxLength: 5 }),
        async (apiChores) => {
