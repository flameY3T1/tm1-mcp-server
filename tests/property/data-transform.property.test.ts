...
describe("Property 5: Strukturierte Datentransformation", () => {
  ...
  it("executeMdx correctly maps all cells, formatted values, and axes", async () => {
    ...
    const cellArb = fc.record({
      ...
      FormattedValue: fc.string({ maxLength: 30 }),
    });
    ...
    const tupleArb = fc.record({
      Members: fc.array(memberArb, { minLength: 1, maxLength: 3 }),
    });
    const axisArb = fc.record({
      Tuples: fc.array(tupleArb, { minLength: 1, maxLength: 5 }),
    });

    await fc.assert(
      fc.asyncProperty(
        fc.array(cellArb, { minLength: 1, maxLength: 10 }),
        fc.array(axisArb, { minLength: 1, maxLength: 2 }),
        async (cells, axes) => {
          const f = vi
            .fn()
            .mockResolvedValue(
              mockResp({ ID: "cs-1", Cells: cells, Axes: axes }),
            );
          ...
          for (let i = 0; i < cells.length; i++) {
            ...
            expect(result.cells[i].formattedValue).toBe(
          ...
          for (let a = 0; a < axes.length; a++) {
            ...
            for (let t = 0; t < axes[a].Tuples.length; t++) {
              expect(result.axes[a].tuples[t].members).toHaveLength(
                axes[a].Tuples[t].Members.length,
              );
              for (let m = 0; m < axes[a].Tuples[t].Members.length; m++) {
                expect(result.axes[a].tuples[t].members[m].name).toBe(
                  axes[a].Tuples[t].Members[m].Name,
                );
                expect(result.axes[a].tuples[t].members[m].hierarchyName).toBe(
                  axes[a].Tuples[t].Members[m].Hierarchy.Name,
                );
              }
            }
          }
  ...
  it("getView correctly maps cubeName, viewName, cells, and axes", async () => {
    const cellArb = fc.record({
      ...
      FormattedValue: fc.string({ maxLength: 20 }),
    });
    const memberArb = fc.record({
    ...
    const axisArb = fc.record({
      Tuples: fc.array(tupleArb, { minLength: 1, maxLength: 3 }),
    });

    await fc.assert(
      fc.asyncProperty(
        fc.array(cellArb, { minLength: 0, maxLength: 6 }),
        fc.array(axisArb, { minLength: 0, maxLength: 2 }),
        async (cells, axes) => {
          const f = vi
            .fn()
            .mockResolvedValue(
              mockResp({ ID: "cs-2", Cells: cells, Axes: axes }),
            );
          const client = makeClient(f);
