...
const noSubset = async () => {
  throw new Error("no subset");
};
...
describe("buildDatasourceMembership", () => {
    ...
    const m = await buildDatasourceMembership(
      {
        ...
        getSubset: async (dim: string, _h: string, sub: string) => ({
          ...
          elements: ["SuDatenquellen_C", "SuDatenquellen_D"],
          alias: undefined,
        }),
      },
      [
        {
          ...
          sourceName: "Datenquellen",
          subset: "sMy",
        },
      ],
    );
    expect(
      m.byElement.get(elementKey("Datenquellen", "SuDatenquellen_C")),
    ).toEqual([{ process: "P", via: "subset-static" }]);
  });
  ...
  it("resolves an MDX view datasource to literal members + flags computed selectors", async () => {
      ...
      [
        {
          name: "P",
          type: "TM1CubeView",
          sourceName: "ZusaetzlicheFahrten",
          view: "vMy",
        },
      ],
    );
    expect(
      m.byElement.get(elementKey("Datenquellen", "SuDatenquellen_C")),
    ).toEqual([{ process: "P", via: "view-mdx" }]);
    // Whole-MDX-VIEW datasource: not tied to a single axis → recorded under the "*" sentinel.
    expect([...(m.computedByProcess.get("P")?.get("*") ?? [])].sort()).toEqual([
      "TM1FILTERBYLEVEL",
      "TM1SUBSETALL",
    ]);
  });
    ...
    const m = await buildDatasourceMembership(
      {
        getViewDefinition: async (cube: string, view: string) => ({
          ...
          native: {
            titles: [
              {
                dimensionName: "Szenario",
                selectedElement: "Ist",
                subsetName: "sScenario",
              },
            ],
            columns: [],
            rows: [],
  ...
  it("records a per-object fetch error without throwing", async () => {
    ...
    expect(m.fetchErrors).toEqual([
      { process: "P", object: "view C/vX", message: "boom" },
    ]);
  });
});
...
describe("buildDatasourceMembership — computed axis resolution (C1)", () => {
            ...
            rows: [
              {
                ...
                expression: "{TM1FILTERBYLEVEL({TM1SUBSETALL([Currency])},0)}",
              },
            ],
          },
        }),
      ...
      [
        {
          name: "P",
          type: "TM1CubeView",
          sourceName: "Cube_Assumptions",
          view: "vC",
        },
      ],
    );
    expect(m.byElement.get(elementKey("Currency", "USD"))).toEqual([
      { process: "P", via: "view-native-computed" },
    ]);
    expect(calls).toEqual([
      {
        cube: "Cube_Assumptions",
        dim: "Currency",
        set: "{TM1FILTERBYLEVEL({TM1SUBSETALL([Currency])},0)}",
      },
    ]);
    // computed selector still recorded (honest provenance) even though resolved
    expect([
      ...(m.computedByProcess.get("P")?.get("currency") ?? []),
    ]).toContain("TM1FILTERBYLEVEL");
  });
  ...
  it("eval failure is recorded in fetchErrors and does not throw", async () => {
            ...
            rows: [
              {
                dimensionName: "Currency",
                hierarchyName: "Currency",
                expression: "{DESCENDANTS([Currency])}",
              },
            ],
          },
        }),
    ...
    expect(m.byElement.size).toBe(0);
    expect(
      m.fetchErrors.some((e) => e.process === "P" && /eval/i.test(e.object)),
    ).toBe(true);
  });
});
