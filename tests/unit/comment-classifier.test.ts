...
describe("isCommentedOutCode", () => {
  ...
  it("flags rule-area assignments and known function calls", () => {
    expect(isCommentedOutCode("#['Revenue','Actual'] = DB('Sales', !x)")).toBe(
      true,
    );
  ...
  it("treats natural-language prose as a real comment", () => {
    expect(
      isCommentedOutCode("# This rule allocates revenue by headcount"),
    ).toBe(false);
    expect(isCommentedOutCode("# TODO: revisit after Q3 close")).toBe(false);
    expect(isCommentedOutCode("# Author: finance team, see ticket 1234")).toBe(
      false,
    );
    expect(isCommentedOutCode("#")).toBe(false);
    expect(isCommentedOutCode("# ----------------------------------")).toBe(
      false,
    );
  });
});
