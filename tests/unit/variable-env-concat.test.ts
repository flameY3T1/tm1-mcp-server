...
describe("resolveExpression — constant concat folding", () => {
  it("folds two string literals", () => {
    expect(resolveExpression("'te' | 'st'", emptyEnv())).toEqual({
      kind: "literal",
      value: "test",
    });
  });
  it("folds three literals", () => {
    expect(resolveExpression("'z' | 'A' | 'B'", emptyEnv())).toEqual({
      kind: "literal",
      value: "zAB",
    });
  });
  it("folds literal concatenated with numeric literal", () => {
    expect(resolveExpression("'v' | 42", emptyEnv())).toEqual({
      kind: "literal",
      value: "v42",
    });
  });
  it("folds a literal concatenated with a bound literal variable", () => {
    ...
    expect(resolveExpression("'zA' | sOther", env)).toEqual({
      kind: "literal",
      value: "zAB",
    });
  });
  it("stays dynamic when one operand is an unknown identifier", () => {
    expect(resolveExpression("'zA' | sOther", emptyEnv())).toEqual({
      kind: "dynamic",
    });
  });
  it("does not split a pipe inside a string literal", () => {
    expect(resolveExpression("'a|b'", emptyEnv())).toEqual({
      kind: "literal",
      value: "a|b",
    });
  });
  it("does not split a pipe inside parens", () => {
    expect(resolveExpression("FOO('a|b')", emptyEnv())).toEqual({
      kind: "dynamic",
    });
  });
...
describe("buildReferenceIndex — constant-concat call resolves edge", () => {
  it("resolves ExecuteProcess(concat) to a real edge, not unresolved", async () => {
    ...
    expect(
      index.bySourceProcess
        .get("p")
        ?.some((r) => r.targetKind === "process" && r.targetName === "zAzB"),
    ).toBe(true);
