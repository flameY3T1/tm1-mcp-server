...
function tsFromFilename(filename: string): number | null {
  const m = filename.match(TS_RE);
  if (!m) return null;
  const s = m[1];
  // YYYYMMDDHHMMSS -> Date.UTC. TM1 emits this in server-local time; treat as UTC for diffing
  // (we only compare differences within one server, so timezone offset cancels out).
...
function truncateTail(content: string, maxBytes: number): { body: string; truncated: boolean } {
