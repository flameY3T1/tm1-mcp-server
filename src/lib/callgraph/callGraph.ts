...
export function resolveEdgeParam(
  ...
  switch (res.kind) {
    case "literal":
      return { kind: "literal", value: res.value };
    case "dynamic":
      return { kind: "dynamic" };
    case "passthrough": {
      const up = parentEnv.get(res.paramName.toLowerCase());
      if (!up) {
        return { kind: "unknown", viaParam: res.paramName };
      }
      if (up.kind === "literal") {
        return up;
      }
      if (up.kind === "unknown") {
        return { kind: "unknown", viaParam: up.viaParam };
      }
      return { kind: "dynamic" };
    }
  }
...
export function buildCallGraph(
  ...
  const rootEnv =
    opts.direction === "downstream" ? buildInitialEnv(start, index) : undefined;
    ...
    edges.sort(
      (a, b) =>
        (opts.direction === "downstream"
          ...
          : a.caller.localeCompare(b.caller)) || a.line - b.line,
    );

    for (const edge of edges) {
      const nextProc =
        opts.direction === "downstream" ? edge.callee : edge.caller;
      if (!includeSystem && nextProc.startsWith("}")) {
        continue;
      }
      const childEnv =
        opts.direction === "downstream" && node.env
...
export function buildCubeOrDimUsages(
  ...
  for (const r of refs) {
    ...
    if (
      kind === "cube" &&
      r.sourceKind === "rule" &&
      r.sourceName.toLowerCase() === nameLc
    ) {
      continue;
    }
    if (!includeSystem && r.sourceName.startsWith("}")) {
      continue;
    }
    const accessType = classifyAccess(r.funcName, r.sourceKind);
    if (accessMode !== "all" && accessType !== accessMode) {
      continue;
    }
    result.push({
  ...
  result.sort(
    (a, b) => a.sourceName.localeCompare(b.sourceName) || a.line - b.line,
  );
  return result;
}

/** Flattens the tree depth-first for simple list rendering. */
export function flattenCallGraph(
  root: CallGraphNode,
): Array<{ node: CallGraphNode; depth: number }> {
...
export function formatEdgeParams(edge: CallEdge): string {
  if (edge.effectiveParams) {
    if (edge.effectiveParams.length === 0) {
      return "";
    }
    ...
    .map((p) => {
      switch (p.resolution.kind) {
        case "literal": {
          ...
          return wasQuoted
            ? `${p.name}='${p.resolution.value}'`
            : `${p.name}=${p.resolution.value}`;
        }
        case "passthrough":
          return `${p.name}=«${p.resolution.paramName}»`;
        case "dynamic":
          return `${p.name}=?`;
      }
    })
